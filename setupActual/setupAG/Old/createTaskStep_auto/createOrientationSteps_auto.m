function [ts_optim1, ts_optim2, ts_sfSweep, ts_ctrSweep, ts_orSweep, ts_durLimited, ts_durLimitedCtr, ts_durSweep] = createOrientationSteps_auto(svnRev,svnCheckMode,subID)
...
constantRewardsWithRequest=constantReinforcement(rewardSize,requestRewardSize,doAllRequests,msPenalty,fractionSoundOn,fractionPenaltySoundOn,rewardScalar,msAirpuff);

tm= nAFC(sm, percentCorrectionTrials, constantRewards);
tmWithRequest= nAFC(sm, percentCorrectionTrials, constantRewardsWithRequest);

% training step using other objects as passed in
ts_optim1 = trainingStep(tmWithRequest, afc_optim, thresholdPC, sch, svnRev, svnCheckMode,'orOptimal');
ts_optim2 = trainingStep(tm, afc_optim, thresholdPC, sch, svnRev, svnCheckMode,'orOptimal');
