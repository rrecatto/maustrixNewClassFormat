function [rigState ampState surgBregma surgAnchor currAnchor currPositn penetParams monitorDetails subjectDetails isNewDay] = getDataFromEventLog(eventLogPath,searchDepth,subjID)
...
penetParams = [];
monitorDetails = {''};
subjectDetails = {subjID,'virtual','wildtype','','none','none'};
...
doCurrPositn = true;
doPenetParams = true;
doMonitorDetails = true;
doSubjectDetails = true;
...
for currSearchDepth = 1:searchDepth
    ...
    while doLoop
        ...
        if ~isfield(events_data,'monitorDetails') 
            doMonitorDetails = false;
        end
        
        if ~isfield(events_data,'subjectDetails') 
            doSubjectDetails = false;
        end
        if doRigState
        ...
        if doMonitorDetails
            if isfield(events_data(i),'monitorDetails') && (~isempty(events_data(i).monitorDetails))
                monitorDetails = events_data(i).monitorDetails;
                doMonitorDetails = false;
            end
        end
        
        if doSubjectDetails
            if ~isempty(events_data(i).monitorDetails)
                subjectDetails = events_data(i).subjectDetails;
                doSubjectDetails = false;
            end
        end
        ...
        if i>1
            i = i-1;
            doLoop = doRigState || doAmpState || doSurgBregma || doSurgAnchor || doCurrAnchor || doCurrPositn || doPenetParams || doMonitorDetails || doSubjectDetails;
        else
            doLoop = false;
        end
    end
