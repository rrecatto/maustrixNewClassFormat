function [physRecords success filePaths]=getPhysRecords(subjectDataPath,when,what,filter)
% INPUTS:
...
try
    ...
    for w=1:length(what)
        ...
        switch what{w}
            ...
            case 'spike'
                  error('format changed... use singleUnit.getSpikes')
%                 %spike and analysis have a non-trial convention... ranges!
%                 goods=[];
...
%                 searchStr='spikeRecords_(\d+)-(.*)\.mat';
            case 'analysis'
                error('format changed... use singleUnit.getAnalysis')
%                 % hard to do
%                 goods=[];
%                 d=dir(fullfile(subjectDataPath,'analysis'));
%                 for i=1:length(d)
...
%                 searchStr='physAnalysis_(\d+)-(.*)\.mat';
    ...
    while ~success && ~isempty(goodFiles)
        ...
        if success
            ...
            %calc all the paths we figured out
            fn=sprintf('stimRecords_%d-%s.mat',goodFiles(end).trialStart,goodFiles(end).dateStart);
            filePaths.stim=fullfile(subjectDataPath,'stimRecords',fn);
            
            fn=sprintf('scratchPad_%d-%s.mat',goodFiles(end).trialStart,goodFiles(end).dateStart);
            filePaths.scratchPad=fullfile(subjectDataPath,'scratchPad',fn);
            
            fn=sprintf('neuralRecords_%d-%s.mat',goodFiles(end).trialStart,goodFiles(end).dateStart);
            filePaths.neural=fullfile(subjectDataPath,'neuralRecords',fn);          
            ...
            filePaths.spike=fullfile(subjectDataPath,'analysis',ff,fn);
            
            fn=sprintf('eyeRecords_%d_%s.mat',goodFiles(end).trialStart,goodFiles(end).dateStart);
            filePaths.eye=fullfile(subjectDataPath,'eyeRecords',fn);
                        
            % load files desired in 'what'
            for w=1:length(what)
                switch what{w}
                    case 'stim'
                        record=load(filePaths.stim);
                    case 'neural'
                        record=load(filePaths.neural)
                    case 'analysis'
                        % can't output this path unless we did some
                        % processing (and we must ask for that)
                    ...
                    case 'spike'
                        record=load(filePaths.spike);
                    case 'eye'
                        record=load(filePaths.eye);
                    otherwise
