...
% fieldsSizeOfNumChunks = {'passedQualityTest','chunkHasFrames','chunkNeuralDataTimes'}; %indexed by trialNum and chunkInd
...
switch filterParams.filterMode
    case {'everythingAvailable'}
        filteredSpikeRecord = spikeRecord;
        
    case {'thisTrialAndChunkOnly','thisTrialOnly','theseTrialsOnly','theseTrialsOnlyForAnalysis'}
        % which?
        ...
        switch filterParams.filterMode
            ...
            case 'thisTrialOnly'
                newFilterParams.filterMode = 'theseTrialsOnly';
                newFilterParams.trialRange = filterParams.trialNum;
                filteredSpikeRecord = filterSpikeRecords(newFilterParams,spikeRecord);
                
            case 'theseTrialsOnly'
                theseTrials = filterParams.trialRange;
                
                % fields size of numChunks
                filterForFieldsSizeNumChunks.exists = false;
                if isfield(spikeRecord,'trialNum')
                    filterForFieldsSizeNumChunks.exists = true;
                    filterForFieldsSizeNumChunks.which = ismember(spikeRecord.trialNum,theseTrials);
                end
                ...
                if isfield(spikeRecord,'trialNumForFrames')
                    filterForFieldsSizeFrames.exists = true;
                    filterForFieldsSizeFrames.which = ismember(spikeRecord.trialNumForFrames,theseTrials);
                end
                ...
                if isfield(spikeRecord,'trialNumForCorrectedFrames')
                    filterForFieldsSizeCorrectedFrames.exists = true;
                    filterForFieldsSizeCorrectedFrames.which = ismember(spikeRecord.trialNumForCorrectedFrames,theseTrials);
                end
                ...
                for currTrode = trodesInRecord'
                    if isfield(spikeRecord.(currTrode{:}),'trialNumForDetectedSpikes')
                        filterForFieldsSizeDetectedSpikes.exists = true;
                        filterForFieldsSizeDetectedSpikes.(currTrode{:}).which = ismember(spikeRecord.(currTrode{:}).trialNumForDetectedSpikes,theseTrials);
                    end
                end
                ...
                for currTrode = trodesInRecord'
                    if isfield(spikeRecord.(currTrode{:}),'trialNumForSortedSpikes')
                        filterForFieldsSizeSortedSpikes.exists = true;
                        filterForFieldsSizeSortedSpikes.(currTrode{:}).which = ismember(spikeRecord.(currTrode{:}).trialNumForSortedSpikes,theseTrials);
                    end
                end
            case 'theseTrialsOnlyForAnalysis'
                theseTrials = filterParams.trialRange;
                ...
                for currTrode = trodesInRecord'
                    if isfield(spikeRecord.(currTrode{:}),'trialNumForDetectedSpikes')
                        filterForFieldsSizeSortedSpikes.exists = true;
                        filterForFieldsSizeSortedSpikes.(currTrode{:}).which = ismember(spikeRecord.(currTrode{:}).trialNumForDetectedSpikes,theseTrials);
                    end
                end
        end
        %%%%%%%%%%%%%%%%%%%%%%%%%
        ...
        if filterForFieldsSizeNumChunks.exists
            fieldsSizeOfNumChunks = {'passedQualityTest','chunkHasFrames','chunkDuration','trialNum','chunkID'}; %indexed by trialNum and chunkInd
        ...
        if filterForFieldsSizeCorrectedFrames.exists
            fieldsSizeOfCorrectedFrames = {'correctedFrameIndices','correctedFrameLengths','correctedFrameTimes',...
                'stimInds','trialNumForCorrectedFrames','chunkIDForCorrectedFrames'}; % indexed by trialNumForCorrectedFrames and chunkIDForCorrectedFrames 'photoDiode',
