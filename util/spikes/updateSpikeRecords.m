...
%                               spikeTimestamps
%                               spikeWaveforms
%                               trialNumForDetectedSpikes
%                               chunkIDForDetectedSpikes
%                               trialNumForSortedSpikes
%                               chunkIDForSortedSpikes
%                               assignedClusters
%                               processedClusters
%                               spikeModel.
%                                          featureDetails
...
switch updateParams.updateMode
    case 'frameAnalysis'
        if ~isempty(currentSpikeRecord.correctedFrameIndices)
            fieldsToUpdateWithoutAdj = {'frameTimes','frameLengths','correctedFrameTimes','correctedFrameLengths',...
                ...
                'trialNumForFrames','chunkHasFrames','trialNum','chunkID','photoDiode','chunkDuration','trialStartTime'};
        ...
        else % the chunk has no frames...update stuff that are not relevant to frames
            fieldsToUpdate = {'passedQualityTest','chunkHasFrames','trialNum','chunkID','chunkDuration','trialStartTime'};
            for currField = fieldsToUpdate
                if ~isfield(spikeRecord,currField{:})
                    spikeRecord.(currField{:}) = [];
                end
                spikeRecord.(currField{:}) = [spikeRecord.(currField{:});currentSpikeRecord.(currField{:})];
