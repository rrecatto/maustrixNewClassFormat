function [detectionAndSortingParamsValidated spikeDetectionParams spikeSortingParams trodes] = ...
    validateAndSetDetectionAndSortingParams(spikeDetectionParams,spikeSortingParams,channelAnalysisMode,trodes,analysisPath,channelConfiguration)
...
switch channelAnalysisMode
    ...
    case 'allPhysChannels'
        ...
        % channels we want to analyze are. alternately it comes in here
        % when we have already analyzed the spikes. and know what the
        % trodes are
        % make sure that channelConfiguration exists
        % try to load trodeInfo from the analysisBoundaryFile
        analysisBoundaryFile = fullfile(analysisPath,'analysisBoundary.mat');
        % maybe trodes already exists?
        if exist(analysisBoundaryFile,'file')
            temp = stochasticLoad(analysisBoundaryFile,{'trodes'},5);
        else
            temp = [];
        end
        if (~isfield(temp,'trodes')||isempty(temp.trodes))&&(~exist('channelConfiguration','var') || isempty(channelConfiguration))
            warning('either trodes shuold already exist in analysisBoundary, or channelConfiguration should be input for channelAnalysisMode:''%s''. returning input values for params',channelAnalysisMode);
            return;
        elseif isfield(temp,'trodes')&& ~isempty(temp.trodes)
            trodes = temp.trodes;
            numTrodes = length(trodes);
        elseif exist('channelConfiguration','var')&&~isempty(channelConfiguration)
