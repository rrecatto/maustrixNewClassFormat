...
switch upper(spikeDetectionMethod)
    ...
    case {'FILTEREDTHRESH','FILTEREDTHRESHSTD'}
        ...
        if ~isfield(spikeDetectionParams, 'thresholdVolts') && ~strcmp(spikeDetectionParams.thresholdMethod,'STD')
            if ~isfield(spikeDetectionParams, 'bottomTopCrossingRate') || isempty(spikeDetectionParams.bottomTopCrossingRate)
                ...
                spikeDetectionParams.thresholdVolts = []; % will be determined from rate
            end
        end
        ...
        if ~isfield(spikeDetectionParams, 'minmaxVolts')
            if length(spikeDetectionParams.thresholdVolts) == 3
                spikeDetectionParams.minmaxVolts = [-spikeDetectionParams.thresholdVolts(3) spikeDetectionParams.thresholdVolts(3)];
            else
                spikeDetectionParams.minmaxVolts = [-Inf Inf];
                warning('minmaxVolts not defined - using default value of [-Inf Inf], which does not remove any high amplitude noise ever');
            end
        ...
        if isfield(spikeDetectionParams, 'bottomTopCrossingRate') && ~isempty(spikeDetectionParams.bottomTopCrossingRate)
            if ~isempty(spikeDetectionParams.thresholdVolts)
                thresholdVolts=spikeDetectionParams.thresholdVolts;
        ...
        if doThreshFromRate
            ...
            disp(sprintf('spikeDetectionParams.thresholdVolts=[%2.3f %2.3f]  %%fit from desired rate',loThresh,hiThresh))
            spikeDetectionParams.thresholdVolts=[loThresh hiThresh]; % for later display
        end
        ...
        if strcmp(spikeDetectionParams.thresholdMethod,'STD')
            ...
            loThresh = mFS+spikeDetectionParams.thresholdVolts(1)*stdFS;
            hiThresh = mFS+spikeDetectionParams.thresholdVolts(2)*stdFS;
