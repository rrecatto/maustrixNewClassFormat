%edf downloaded this from: http://www.mathworks.com/matlabcentral/fileexchange/13845
%and adaptd to work for vertical sliders (very quck/dirty/ugly!)
...
if nargin > 1 && ~iscell(PanelPVs) && ~isa(PanelPVs,'struct')
	...
	PanelPVs = validate(allargs,PanelPVs,...
		...
		'fontweight','backgroundcolor','bordertype','tag','units','visible'});
...
if iscell(PanelPVs)
    PanelPVs=struct(PanelPVs{:});
end
if isfield(PanelPVs,'szs') %assumes vertical
    szs=PanelPVs.szs;
    ind=PanelPVs.ind;
    
    set(panelHandle,'units','pixels');
    set(panelHandle,'pos',[szs.border+(ind-1)*(4*szs.border+szs.stWidth+szs.sWidth) szs.border 3*szs.border+szs.sWidth+szs.stWidth 2*szs.border+szs.sHeight]);
    
    PanelPVs=rmfield(PanelPVs,'szs');
    PanelPVs=rmfield(PanelPVs,'ind');
    
    if iscell(SliderPVs)
        SliderPVs=struct(SliderPVs{:});
    end
    
    SliderPVs.units='pixels';
    SliderPVs.pos=[2*szs.border+szs.stWidth szs.border szs.sWidth szs.sHeight-2*szs.border];
end
applyPVs(panelHandle,PanelPVs);

% CREATE SLIDER
sliderHandle = uicontrol(panelHandle,'style','slider');
applyPVs(sliderHandle,SliderPVs);

panelPos=get(panelHandle,'pos');

if panelPos(3)>panelPos(4)
    editPos=[0.35 0.05 0.3 0.35];
    labPos{1}=[0.05 0.05 0.25 0.25];
    labPos{2}=[0.7 0.05 0.25 0.25];
    us='normalized';
    align={'l','r'};
else
    us='pixels';
    editPos=  [szs.border szs.sHeight/2                       szs.stWidth szs.stHeight];
    labPos{1}=[szs.border szs.border                          szs.stWidth szs.stHeight];
    labPos{2}=[szs.border szs.sHeight-szs.border-szs.stHeight szs.stWidth szs.stHeight];
    align={'c','c'};
end

% CREATE EDIT BOX (DEFAULT)
editHandle = uicontrol(panelHandle,'style','edit','units',us,...
	'pos',editPos);
% CREATE LABELS (DEFAULT)
labelHandle(1) = uicontrol(panelHandle,'style','text','units',us,...
	'pos',labPos{1},'horizontalalignment',align{1},...
	'Backgroundcolor',get(panelHandle,'Backgroundcolor'));
labelHandle(2) = uicontrol(panelHandle,'style','text','units',us,...
	'pos',labPos{2},'horizontalalignment',align{2},...
	'Backgroundcolor',get(panelHandle,'Backgroundcolor'));
