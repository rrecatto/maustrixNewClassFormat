...
switch requestedDetailsType
    case NUM_CHUNKS
        if isempty(fieldsTemp)||isempty(temp.numChunks) % there cant be a neural record without any chunks...impossible!
    ...
    case SPECIFIC_CHUNK
        if isempty(fieldsTemp)
            if strcmp(requestedDetails,'chunk0')
                details = load(neuralRecordLocation);
            else
