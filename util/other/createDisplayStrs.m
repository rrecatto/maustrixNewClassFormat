    ...
    case 'full'
        for i=length(toShow):-1:1
            switch toShow(i).eventType
                case {'Prelim','Summary','comment','top of fluid','top of brain','ctx cell','hipp cell','deadzone','theta chatter','visual hash','visual cell',...
                ...
                case {'muscimol'}
                    if isfield(toShow(i).eventParams,'concentration');
                        concentration  = toShow(i).eventParams.concentration;
                        volume = toShow(i).eventParams.volume;
                    else
                        concentration = 'unknown';
                        volume = 'unknown';
                    end
                    str = sprintf('%s%sadded %s uL muscimol @ %s mg/ml',datestr(toShow(i).time,'HH:MM'),fixedTab,volume,concentration);
                otherwise
                    toShow(i)
