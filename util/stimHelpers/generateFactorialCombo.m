...
elseif iscell(mode)
    ...
    if (length(mode)==1 && strcmp(mode{1}, 'ordered'))||(length(mode)==2 && strcmp(mode{1}, 'ordered')&&(isempty(mode{2})))
        % 'ordered' mode
        type='ordered';
    elseif ismember(mode{1}, {'random','twister','state','seed'})
        % 'random' mode
        type='random';
    else
        mode
...
switch type
    case 'random'
        ...
        if strcmp(mode{1},'random')
            randMethod = 'twister';
        else
            randMethod = mode{1};
        end
        oldSeed=rand(randMethod);
        rand(randMethod,seed);
        order=randperm(size(retval,2));
        retval=retval(:,order);
        % now restore the oldSeed
        rand(randMethod,oldSeed);
    otherwise
        % do nothing
end
