function [out details] = createAnalysisScript(ratID,dataPath,eventLogCommentingON,analysisParamFileName,writeScript)
% try
    %% INPUT VERIFICATION
    ...
    if ~exist('dataPath','var')||isempty(dataPath)
        dataPath = '\\132.239.158.169\datanet_storage';
    end
    ...
    if ~exist('writeScript','var')||isempty(writeScript)
        writeScript = true;
    end
...
%% the rest
    ...
    for currTrialNumber = 2:length(ratStimContents)
        ...
        if ~stopAnalysisStreak
            ...
            details.type{counter}=explanationString;
            details.trialRange(counter,:)=[currStartStreak currStopStreak];
