function out = getBasicParams
devOn = false;
%% set path for neural and trial Records

if IsWin
    path = '\\EPHYS-DATA-PC\PhysData\physiologyData';
    recordsPaths.neuralRecordLoc = path;
    recordsPaths.stimRecordLoc = path;
    recordsPaths.eyeRecordLoc = path;
    recordsPaths.analysisLoc = path;
elseif IsLinux
    path = '/home/balaji/datanetOutput';
    recordsPaths.neuralRecordLoc = path;
    recordsPaths.stimRecordLoc = path;
    recordsPaths.eyeRecordLoc = path;
    recordsPaths.analysisLoc = path;
elseif ismac
    path = '/Volumes/BAS-DATA1/physiology/datanetOutput';
    recordsPaths.neuralRecordLoc = path;
    recordsPaths.stimRecordLoc = path;
    recordsPaths.eyeRecordLoc = path;
    recordsPaths.analysisLoc = path;    
...
out.recordsPaths = recordsPaths;

%% spikeDetectionParams
spikeDetectionParams.method = 'filteredThreshSTD'; %'filteredThresh'; %'oSort'; %
% fill in the details
switch spikeDetectionParams.method
    ...
    case 'filteredThreshSTD' %use filteredThresh
        spikeDetectionParams.freqLowHi = [200 10000];
        spikeDetectionParams.threshHoldVolts = [-3 3];
        spikeDetectionParams.minmaxVolts = [-50 50];
        spikeDetectionParams.waveformWindowMs= 2.5;
        spikeDetectionParams.peakWindowMs= 1;
        spikeDetectionParams.alignMethod = 'atPeak'; %atCrossing
        spikeDetectionParams.peakAlignment = 'filtered'; % 'raw'
        spikeDetectionParams.returnedSpikes = 'filtered'; % 'raw'
        spikeDetectionParams.lockoutDurMs=0.1; %spikeDetectionParams.waveformWindowMs-spikeDetectionParams.peakWindowMs; %0
        spikeDetectionParams.thresholdMethod = 'STD';
    otherwise
        error('unsupported spikeDetection method');
end
spikeDetectionParams.ISIviolationMS=2; % just for human reports
%% LFP
spikeDetectionParams.sampleLFP = true; %true;%false
spikeDetectionParams.LFPSamplingRateHz =500;
if devOn
    out.spikeDetectionParams = detectionParam(spikeDetectionParams);
else
...
if devOn
    out.spikeSortingParams = sortingParam(spikeSortingParams);
else
...
otherParams.subSampleSpikes = false;
otherParams.subSampleProb = NaN;
otherParams.milliSecondPrecision = false;
otherParams.pixelOfInterest = [NaN NaN];
otherParams.precisionInMS = 1;
...
if devOn
    otherParams.stimClassToAnalyze = out.stimClassToAnalyze;
    otherParams.timeRangePerTrialSecs = out.timeRangePerTrialSecs;
    otherParams.makeBackup = out.makeBackup;
end
% monitor
otherParams.monitor = monitor('WestinghouseL2410NM');
% rig
rS.calcMethod = 'default';
rS.monitor = 'WestinghouseL2410NM';
otherParams.rigState = rig('default',rS);
