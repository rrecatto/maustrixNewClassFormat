function [out fval flag] = radDOGFit(in)
tic;
...
switch in.searchAlgorithm
    case {'fminsearch','fmincon','fmincon-balanced','fmincon-suprabalanced','fmincon-subbalanced','fmincon-notsubbalanced',...
        ...
        [rcInit rsInit kcInit ksInit] = guessInitialValue(in.rs,in.fs,in.f1,in.initialGuessMode,initialGuessParams);
        ...
        switch in.searchAlgorithm
            case 'fminsearch'
                ...
                [out fval flag] = fminsearch(@(p) dogError(p,rs,fs,f1,dogErrorParams),pInit,opt);
                % exit check
...
function er = dogError(p,rs,fs,f1,params)
if ~exist('params','var')||isempty(params)
...
switch params.model
    case '1D-DOG'
        ...
        f1FitOut = radGratings(rf,stim,mode);
