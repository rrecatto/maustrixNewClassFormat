...
%recommended use:  go cell by cell as you might want to adjust parameters
%without reloading all data again
...
eventFont=12;
bLen=2000; %try 1000-5000; block length for chuncked anlayis, 1 block = 1 number in matrix
maxTrialsViewed=70000; %try 50,000-250,000;  to view an interesting range
bottom=maxTrialsViewed./bLen;
...
for j=1:length(eventInds)
    ...
    for i=1:length(subjects)
        ...
        if isempty(postEvent) || postEvent==min(find(D.subject==subjects(i)))
            ...
            if trailsByThen<maxTrialsViewed & trailsByThen>bLen %crop edge numbers
...
for i=1:length(subjects)
    ...
    bEnd=min(numTrials-mod(numTrials,bLen),maxTrialsViewed-bLen);
...
%% save
%resize the figure to asthetic taste and run this cell again
m=getFrame(gcf);
path='C:\Documents and Settings\rlab\Desktop'
imwrite(m.cdata, fullfile(path,'megaLick.jpg'), 'jpg','Quality' ,100) %green saturation sucks...
imwrite(m.cdata, fullfile(path,'megaLick.png'), 'png')