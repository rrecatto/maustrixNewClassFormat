...
%%
        ...
        function viewOnGoingFit(p,hit,miss,fa,cr,data,x,params,handles,includeSubjectData)
            ...
            if 0 % not used now
                ...
                switch p.sdtDistributionType
                    case {'gaussian','eqVarGaussian'}
                        critSweep=linspace(-1,3,20);
                        h=1-normcdf(critSweep,params.muS(cInd),params.sigmaS(cInd));
                        f=1-normcdf(critSweep,params.muN(cInd),params.sigmaN(cInd));
                    case 'gev'
                        critSweep=linspace(-2,3,30);
                        h=1-gevcdf(critSweep,params.shapeS(cInd),params.sigmaS(cInd),params.muS(cInd));
                        f=1-gevcdf(critSweep,params.shapeN(cInd),params.sigmaN(cInd),params.muN(cInd));
                    otherwise
                        error('bad')
                end
