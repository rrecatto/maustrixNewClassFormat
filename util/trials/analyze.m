...
if Detect
    for tr = inputParams.trialRange
        ...
        if neuRecExists
            [spikeRecExists, ~, spikeRecPath] = findRecord(inputParams.recordsPath.spikeRecords, tr, 'spike',neuTimestamp);
            oldSpikeDetParam = stochasticLoad(spikeRecPath,'detectionParams');
        else
            spikeRecExists = false;
        end
...
if length(d)==1
    ...
    recordExists = true;
    recordPath = fullfile(path,d(1).name);
elseif length(d)>1
