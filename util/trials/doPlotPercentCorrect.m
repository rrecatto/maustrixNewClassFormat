...
if plotCorrectionTrialsToo
    ...
    UserData.CTplot = plot(find(goodCTs),performance,'color',[1,0.8,0.9]); %pink
end
...
if plotAfterErrorsToo
    ...
    UserData.AEplot = plot(find(goodAEs),performance,'color',[0.8,1,.9]); %light green
end
...
UserData.Perfplot = plot(find(which),performanceOnGood,'color',[0,0,0], 'linewidth', 2);
...
% ## add on reinforcement changes if they exist
rewChanges = find(diff(d.potentialRewardScalar));
penChanges = find(diff(d.potentialPenalty));

if length(rewChanges) ~= 0
    ...
    h=plot(rewChanges,repmat(markerHeight,1,length(rewChanges)), 'x','Color', [0 .8 0]);
    set(h, 'markerSize',6,'LineWidth', 2);
    
end

if length(penChanges) ~= 0
    ...
    h=plot(penChanges,repmat(markerHeight,1,length(penChanges)), 'x','Color', [.8 0 0]);
    set(h, 'markerSize',6,'LineWidth', 2);
