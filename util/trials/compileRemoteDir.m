...
for i = 1:length(boxes)
    ...
    failuresThis = compileDetailedRecords([],miceToCompile,recompile,fullfile(currBoxDir,'Permanent'),fullfile(currBoxDir,'Compiled'));
    for j = 1:length(failuresThis)
        failures{end+1} = sprintf('%s :: %s',boxes{i},failuresThis{j});
    end
...
if ~isempty(failures)
    gmail('sbalaji1984@gmail.com','Failures in compileRemoteDir', failures);
end